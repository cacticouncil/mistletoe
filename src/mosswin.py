	...
	def Run(self, studentFiles, baseFiles):
		...
		try:
			...
			for i in range (0, numBaseFiles):
				self.Output("Uploading [{}/{}] '{}'".format(i + 1, numBaseFiles, baseFiles[i]))
				self.MossUploadFile(baseFiles[i], 0)

			numStudentFiles = len(studentFiles)
			for i in range (0, numStudentFiles):
				self.Output("Uploading [{}/{}] '{}'".format(i + 1, numStudentFiles, studentFiles[i]))
			...
			self.Output("Waiting for result...")
			resultUrl = self.RecvLine()
			self.Output("Result: {}".format(resultUrl))
			self.MossEnd()
			self.Shutdown()
			if len(resultUrl) == 0:
